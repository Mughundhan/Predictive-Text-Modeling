...
shinyServer(function(input, output) {
        ...
        output$top11 <- renderDataTable({
          Predicted_Word <- c(textDisplay()[1],
                         textDisplay()[2],
                         textDisplay()[3],
                         textDisplay()[4],
                         textDisplay()[5],
                         ...
                         textDisplay()[10])
          Rank_Order <- c(1:10)
          topdt <- cbind(Rank_Order, Predicted_Word)
          topdt <- as.data.table(topdt)
        })
        ...
        output$plot1 <- renderPlot({
                ...
                wordcloud(words, freq, scale = c(7,2), colors=brewer.pal(8, "Dark2"),
                          fixed.asp = FALSE, rot.per = 0, max.words = input$xPlot)
        })
